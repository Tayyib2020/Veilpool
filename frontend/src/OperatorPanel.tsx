...
import { contractsConfigured, contractConfig, explorerTxUrl } from "./config/contracts";
    ...
    try {
      ...
      switch (action) {
        case "request_principal_deployment":
          setWalletWaiting();
          tx = await contracts.engine.requestPrincipalDeployment();
          break;
        case "finalize_principal_deployment": {
          const handle = String(await contracts.engine.confidentialPendingPrincipalLiability());
          ...
          setWalletWaiting();
          tx = await contracts.engine.finalizePrincipalDeployment(decrypted.value, decrypted.decryptionProof);
          break;
        }
        case "finalize_principal_unwrap": {
          ...
          setWalletWaiting();
          tx = await contracts.engine.finalizePrincipalUnwrap(state.pendingPrincipalUnwrapRequest, decrypted.value, decrypted.decryptionProof);
          break;
        }
        case "restore_principal_liquidity":
          setWalletWaiting();
          tx = await contracts.engine.restorePrincipalLiquidity();
          break;
        case "harvest_yield":
          setWalletWaiting();
          tx = await contracts.engine.harvestYield();
          break;
        case "lock_round":
          setWalletWaiting();
          tx = await contracts.engine.lockRound();
          break;
        case "request_aggregate_decryption":
          setWalletWaiting();
          tx = await contracts.engine.requestAggregateDecryption();
          break;
        case "finalize_aggregate_reveal": {
          const handle = String(await contracts.engine.confidentialTotalWeight());
          const decrypted = await publicDecryptHandle(wallet.ethereum, handle);
          if (typeof decrypted.value !== "bigint") throw new Error("Aggregate proof returned an unsupported value.");
          setWalletWaiting();
          tx = await contracts.engine.finalizeAggregateReveal(decrypted.value, decrypted.decryptionProof);
          break;
        }
        case "commit_harvested_yield": {
          ...
          setWalletWaiting();
          tx = await contracts.engine.commitHarvestedYield(amount);
          break;
        }
        case "close_empty_round":
          setWalletWaiting();
          tx = await contracts.engine.closeEmptyRound();
          break;
        case "cancel_no_yield_round":
          setWalletWaiting();
          tx = await contracts.engine.cancelNoYieldRound();
          break;
        case "execute_draw":
          setWalletWaiting();
          tx = await contracts.engine.executeDraw();
          break;
        case "request_draw_acceptance":
          setWalletWaiting();
          tx = await contracts.engine.requestDrawAcceptance();
          break;
        case "finalize_draw_acceptance": {
          const handle = String(await contracts.engine.confidentialHasAccepted());
          const decrypted = await publicDecryptHandle(wallet.ethereum, handle);
          if (typeof decrypted.value !== "boolean") throw new Error("Draw acceptance proof returned an unsupported value.");
          setWalletWaiting();
          tx = await contracts.engine.finalizeDrawAcceptance(decrypted.value, decrypted.decryptionProof);
          break;
        }
        case "start_next_round":
          setWalletWaiting();
          tx = await contracts.engine.startNextRound();
          break;
      }
